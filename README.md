...
通常は `start.bat` を使えば大丈夫です。  
すでにイメージがある場合は `docker compose up -d` 相当で起動するので、普段使いではかなり速くなります。

初回だけは以下を行うので時間がかかります。

- Docker イメージのビルド
- Python パッケージのインストール
- Qwen3-TTS / Whisper モデルのダウンロード

`start.bat` は、初回だけビルド付き起動、2回目以降は通常起動を行います。  
